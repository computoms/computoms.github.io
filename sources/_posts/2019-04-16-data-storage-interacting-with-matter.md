...
To transform electricity into a mechanical motion, it is simply possible to use the same device (a current generator), but instead of moving the magnet into the coil, we input a current inside the coil to make the magnet rotate. This is how an electric motor works.
